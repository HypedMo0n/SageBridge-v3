...
export default function NewInvoicePage() {
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-6">
      <Link href={customerId ? `/customers/${customerId}` : '/invoices'} className="inline-flex items-center gap-1.5 text-sm text-zinc-500 dark:text-zinc-400 mb-4">
        <ArrowLeft size={16} />
        Back
      </Link>
      <h1 className="text-2xl font-bold tracking-tight">New invoice</h1>
      <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">The customer is already selected from their profile.</p>

      <div className="mt-5 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-900/20 dark:text-amber-300">
        <Info size={20} className="mt-0.5 shrink-0" />
        <p>Invoice creation will activate after the connector supports the invoice.create write contract. No invoice will be posted yet.</p>
      </div>
      ...
      <div className="mt-5 space-y-4 opacity-70">
        ...
        <Field label="Description" placeholder="Work completed" />
